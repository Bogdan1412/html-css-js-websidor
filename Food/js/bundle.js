...
function slider({container, slide, nextArrow, prevArrow, totalCounter,currentCounter, wrapper, field}) {
	const slides = document.querySelectorAll(slide),
		...
		width = parseFloat(window.getComputedStyle(sliderWrapper).width),
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function tabs(tabsSelector, tabsSelectorContent, tabsParentSelector, activeClass) {
...
const getResource = async (url) => {
	...
	if (!res.ok) {
		throw new Error(`Could not fetch ${urn}, status: ${res.status}`)
	}
	return await res.json()
}
...
document.addEventListener('DOMContentLoaded', () => {
	...
	;(0,_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('.timer', '2024-12-14')
